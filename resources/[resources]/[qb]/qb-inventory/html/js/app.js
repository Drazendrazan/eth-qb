...
$(document).on("click", "#protected-menu", function(e) {
    $.post(`https://${GetParentResourceName()}/openProtected`);
});
...
//             var fromQuality = {quality : {}};
//             var toQuality = {quality : {}};
//             allowedItems = ["phone", "jewelry_ring", "jewelry_relic"];
//             if (fromSlot == toSlot && fromInventory == toInventory) {
//                 return;
//             }
//             if (
...
//     });
    ...
    $(".item-slot").droppable({
        ...
        drop: function(event, ui) {
            ...
            allowedItems = ["10kgoldchain", "goldchain", "diamond_ring","rolex"];
            allowedItemsProtected = ["keepcompanionhusky", "keepcompanionraccoon","keepcompanionpoodle","keepcompanionrottweiler","keepcompanionwesty","keepcompanionmtlion","keepcompanionmtlion2","keepcompanioncat","photo",""];
            if (fromSlot == toSlot && fromInventory == toInventory) {
                return;
            }
            if (
                ...
                if (otherLabel.toLowerCase() == "protected"){

                    if (toDataWeight) {
                        if (!allowedItemsProtected.includes(toDataWeight.name)) {
                            return;
                        }
                    }
                    if (allowedItemsProtected.includes(fromDataWeight.name)) {
                        if (
                            updateweights(
                                fromSlot,
                                toSlot,
                ...
                }else if(otherLabel.toLowerCase() == "jewelry"){
...
function checkArray(arr, subArr) {
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, $fromQuality, $toQuality) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            toData.name == fromData.name && checkArray(toData.info.quality, fromData.info.quality)
            && !fromData.unique  
        ) {
            ...
            if (fromData.amount == $toAmount) {
                ...
                // if (newData.name.split("_")[0] == "weapon") {
                    // if (!Inventory.IsWeaponBlocked(newData.name)) {
                        ...
                        if (NewQuality > 75) {
                            QualityColor = "#00f8b9";
                        } else if (NewQuality > 50 && NewQuality < 75) {
                            QualityColor = "#0DBA9C";
                        } else if (NewQuality > 25 && NewQuality < 50) {
                            QualityColor = "#109F87";
                        }
                        else if (NewQuality < 25 && NewQuality > 0) {
                            QualityColor = "#0E525F";
                        }
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    // && checkArray(toData.info.quality, fromData.info.quality)
                ) {
