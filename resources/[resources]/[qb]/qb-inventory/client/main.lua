...
RegisterNUICallback('openProtected', function(cData, cb)
    local Player = QBCore.Functions.GetPlayerData()
    closeInventory()
    Wait(100)
    openAnim()
    TriggerEvent("inventory:client:SetCurrentStash", "Protected-"..Player.citizenid)
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "Protected-"..Player.citizenid, {
    maxweight = 30000, slots = 5,})
    cb('ok')
end)
