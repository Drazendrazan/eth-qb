...
exports("usingController", function() return usingController end)


TriggerEvent('qb-hud:SetValues')
    ...
    if not IsPedSwimmingUnderWater( plyPed ) and oxygenLevel < 25.0 then
      if GetGameTimer() - lastDamageTrigger > 3000 then
        ...
        if oxygenLevel <= 0 then
          ...
          if exports['eth-misc']:GetDeathStatus() then
...
RegisterCommand('testsets', function()
    nos = 10
end)

RegisterNetEvent('qb-hud:nosactive', function(status)
    if status then
        nos = nos
    else
        nos = false
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            -- local pRadio = Player(src).state.radioChannel
            ...
            if healthLevel < 1 then healthLevel = 1 end
...
function hasenhancements()
    return {db = db, wind = wind, ampul = ampul, dollar = dollar, yuzme = yuzme, gym = gym, dev = dev, debug = debug, health = health2, armor = armor2, hunger = hunger2, thirst = thirst2, oxy = oxy2}
end

RegisterNetEvent('qb-hud:pursuitModeHUD')
AddEventHandler('qb-hud:pursuitModeHUD', function(value)
    dobuffthing('pursuitmode', value)
end)

RegisterNetEvent('qb-hud:setHealth')
AddEventHandler('qb-hud:setHealth', function()
    healthLevel = 0
end)
...
RegisterNetEvent("client:newStress", function(status, amount)
    ...
    if status == true then
        ...
        Wait(1000)
        TriggerServerEvent('hud:server:GainStress', pStress)
    elseif status == false then
        pStress = pStress - actualamount
        Wait(1000)
        TriggerServerEvent('hud:server:RelieveStress', pStress)
    end
end)
...
RegisterNetEvent('desirerp:admin:maxstats', function()
    ...
    TriggerServerEvent("qb-death:reviveSV", source)
...
RegisterCommand('ui-r', function()
    ...
    TriggerEvent('qb-business:refresh')
...
RegisterCommand("hudtest", function()
    dobuffthing("dogalgaz", 100)
    dobuffthing("db", 100)
    dobuffthing("wind", 100)
    dobuffthing("lightbulp", 100)
    dobuffthing("swimming", 100)
    dobuffthing("gym", 100)
    dobuffthing("dev", 100)
    dobuffthing("debug", 100)
    dobuffthing("hasregen", true)
    dobuffthing("armor", 1)
    dobuffthing("hunger", 100)
    dobuffthing("thirst", 100)
    dobuffthing("nuke", 100)
    dobuffthing("boostingtracker", 100)
    dobuffthing("dollar", 100)
    hasenhancements("health")
    hasenhancements("armor")
    nos = 100
end)
