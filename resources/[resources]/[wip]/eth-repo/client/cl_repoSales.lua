		...
		while true do
			...
			for i = 1, #Config.VehiclePositions, 1 do
				if Config.VehiclePositions[i]["entityId"] ~= nil then
					...
					local vehProps = QBCore.Functions.GetVehicleProperties(Config.VehiclePositions[i]["entityId"])
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	QBCore.UI.Menu.Open('default', GetCurrentResourceName(), 'sell_veh',
		{
		...
		function(data, menu)
			...
			if action == "buy" then
				QBCore.Functions.TriggerCallback('tp_repocarsales:buyVehicle', function(isPurchasable, totalMoney)
				...
				QBCore.Functions.GetVehicleProperties(veh), price)
			end
...
function RemoveVehicles()
	...
	for i = 1, #VehPos, 1 do
		local veh, distance = QBCore.Functions.GetClosestVehicle(VehPos[i])
...
function SpawnVehicles()
	local VehPos = Config.VehiclePositions
	QBCore.Functions.TriggerCallback("tp_repocarsales:retrieveVehicles", function(vehicles)
		for i = 1, #vehicles, 1 do
			...
			QBCore.Functions.SetVehicleProperties(VehPos[i]["entityId"], vehicleProps)
