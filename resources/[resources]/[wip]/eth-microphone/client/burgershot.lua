...
function checkForHeadset()
    ...
    hasHeadset = true
    TriggerEvent("eth-voice:drivethru:connect", true)
end

function turnOffHeadset()
    hasHeadset = false
    TriggerEvent("eth-voice:drivethru:disconnect", true)
end

function enterDriveThru()
    if hasHeadset then return end
    TriggerEvent("eth-voice:drivethru:setPowerState", true)
    TriggerEvent("eth-voice:drivethru:connect")
end 

function exitDriveThru()
    if hasHeadset then return end
    TriggerEvent("eth-voice:drivethru:disconnect")
end
