...
function ConnectToDTFrequency(pIsHeadset)
    local subscribers = RPC.execute("eth-voice:drivethru:subscribe") --ran by source user
...
function DisconnectFromDTFrequency()
    ...
    RPC.execute("eth-voice:drivethru:unsubscribe") --ran by source user
    DTIsHeadset = false
...
function LoadDTModule()
    ...
    exports["eth-callbacks"]:registerKeyMapping("", "DriveThru Headset", "Push-To-Talk", "+transmitToDriveThru", "-transmitToDriveThru")
    ...
    exports["eth-callbacks"]:registerKeyMapping("", "DriveThru Headset", "On / Off", "+toggleDTRadioState", "-toggleDTRadioState")
    ...
    exports["eth-callbacks"]:registerKeyMapping("", "DriveThru Headset", "Volume Up", "+increaseDTVolume", "-increaseDTVolume")
    RegisterCommand("+increaseDTVolume", IncreaseDTRadioVolume, false)
    RegisterCommand("-increaseDTVolume", function() end, false)

    exports["eth-callbacks"]:registerKeyMapping("", "DriveThru Headset", "Volume Down", "+decreaseDTRadioVolume", "-decreaseDTRadioVolume")
    ...
    RegisterNetEvent("eth-voice:drivethru:connect", ConnectToDTFrequency)

    RegisterNetEvent("eth-voice:drivethru:disconnect", DisconnectFromDTFrequency)

    RegisterNetEvent("eth-voice:drivethru:setPowerState", SetDTRadioPowerState)

    RegisterNetEvent("eth-voice:drivethru:addSubscriber", AddDTSubscriber)

    RegisterNetEvent("eth-voice:drivethru:removeSubscriber", RemoveDTSubscriber)

    -- scuffed way of overriding vanilla keybind
    AddEventHandler("eth:voice:transmissionStarted", function()
        if DTIsHeadset then return end
        StartDTTransmission()
    end)

    AddEventHandler("eth:voice:transmissionFinished", function()
        if DTIsHeadset then return end
        StopDTTransmission(true)
    end)
