...
AddEventHandler("mumbleConnected", function()
    ...
    TriggerEvent("eth:voice:state", true)
end)

AddEventHandler("mumbleDisconnected", function()
    print("Mumble: Disconnected")
    TriggerEvent("eth:voice:state", false)
end)

AddEventHandler("eth:voice:state", function (state)
    VoiceEnabled = state

    TriggerServerEvent("eth:voice:connection:state", state)
...
function AddPlayerToTargetList(serverID, context, transmit)
    if not Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("eth:voice:transmission:state", serverID, context, true)
        end
...
function RemovePlayerFromTargetList(serverID, context, transmit, refresh)
    if Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("eth:voice:transmission:state", serverID, context, false)
        end
...
function AddGroupToTargetList(group, context, pIsInVehicle)
    ...
    TriggerServerEvent("eth:voice:transmission:state", group, context, true, nil, pIsInVehicle)
end
...
function RemoveGroupFromTargetList(group, context)
    ...
    TriggerServerEvent("eth:voice:transmission:state", group, context, false)
end
...
function SetVoiceProximity(proximity)
    ...
    TriggerEvent("eth:voice:proximity", proximity)
...
RegisterNetEvent("eth:voice:transmission:state")
AddEventHandler("eth:voice:transmission:state", function(serverID, context, transmitting, effect, isSourceInVehicle)
...
RegisterNetEvent("eth:voice:targets:player:add")
AddEventHandler("eth:voice:targets:player:add", AddPlayerToTargetList)

RegisterNetEvent("eth:voice:targets:player:remove")
AddEventHandler("eth:voice:targets:player:remove", RemovePlayerFromTargetList)

RegisterNetEvent("eth:voice:refresh")
AddEventHandler("eth:voice:refresh", RefreshConnection)

RegisterNetEvent("eth:voice:proximity:override")
AddEventHandler("eth:voice:proximity:override", function(pId, pMode, pRange, pPriority)
...
RegisterNetEvent("eth-voice:setTransmissionDisabled", function (pType, pDisabled)
...
Citizen.CreateThread(function()
    exports["eth-callbacks"]:registerKeyMapping("", "Voice", "Proximity / Range Toggle", "+cycleProximity", "-cycleProximity", "Z")
    RegisterCommand("+cycleProximity", CycleVoiceProximity, false)
    RegisterCommand("-cycleProximity", function() end, false)

    exports["eth-callbacks"]:registerKeyMapping("", "Voice", "Reset Mumble VOIP", "+mumble", "-mumble", "")
...
RegisterNetEvent("eth-voice:setVoiceProximity")
AddEventHandler("eth-voice:setVoiceProximity", function(pProximity)
...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        if (isTalking and not isTalkingToggle) or IsTalkingOnRadio and not isTalkingOnRadio then
            ...
            TriggerEvent("eth:voice:transmissionStarted", { phone = true, radio = IsTalkingOnRadio })
        elseif (not isTalking and isTalkingToggle) or  isTalkingOnRadio and not IsTalkingOnRadio then
            ...
            TriggerEvent("eth:voice:transmissionFinished", { phone = false, radio = IsTalkingOnRadio })
        end
