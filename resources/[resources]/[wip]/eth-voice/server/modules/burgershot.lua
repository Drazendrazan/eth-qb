...
RPC.register("eth-voice:drivethru:subscribe",function() --source is passed
    print(source)
...
function addPlayerToRadio(pServerId, pFrequency)
    ...
    for _, subscriber in ipairs(activeChannels[pFrequency]["subscribers"]) do
        TriggerClientEvent("eth:voice:radio:added", subscriber, pFrequency, pServerId)
    end
    ...
    TriggerClientEvent("eth:voice:radio:connect", pServerId, pFrequency, activeChannels[pFrequency]["subscribers"])
end
...
function removePlayerFromRadio(pServerId, pFrequency)
    ...
    if #activeChannels[pFrequency]["subscribers"] == 0 then
        ...
        for _, subscriber in ipairs(activeChannels[pFrequency]["subscribers"]) do
            TriggerClientEvent("eth:voice:radio:removed", subscriber, pFrequency, pServerId)
        end
    end
